...
mynest::iaf_psc_exp_semd::Parameters_2::Parameters_2()
  : Tau_( 10.0 )             // in ms
  , C_( 250.0 )              // in pF
  , t_ref_( 2.0 )            // in ms
  , E_L_( -70.0 )            // in mV
...
void
mynest::iaf_psc_exp_semd::Parameters_2::get( DictionaryDatum& d ) const
{
...
double
mynest::iaf_psc_exp_semd::Parameters_2::set( const DictionaryDatum& d )
{
...
void
mynest::iaf_psc_exp_semd::State_::get( DictionaryDatum& d, const Parameters_2& p ) const
{
...
void
mynest::iaf_psc_exp_semd::update( Time const& origin, const long from, const long to )
{
  assert(
  ...
  for ( long lag = from; lag < to; ++lag )
  {
	...
	if (S_.i_syn_ex_ > S_.i_old_ )			
	  S_.multiplicator_ = S_.multiplicator_ - S_.i_syn_in_;
	else if (S_.i_syn_ex_ < 0.001)
	  S_.multiplicator_ = 0;
	//if (S_.i_syn_ex_ >= 0.001 && S_.multiplicator_==0 && S_.i_old_ < 0.001)			old code
	  //S_.multiplicator_ = -S_.i_syn_in_;
	//else if (S_.i_syn_ex_ < 0.001)
	  //S_.multiplicator_ = 0;

  //std::cout << S_.i_syn_ex_ << " , " << S_.i_syn_in_ << " , " << S_.multiplicator_ << " , " << S_.i_old_ << '\n' ;
	  
    if ( S_.r_ref_ == 0 ) // neuron not refractory, so evolve V
      S_.V_m_ = S_.V_m_ * V_.P22_ + S_.i_syn_ex_ * V_.P21ex_ * S_.multiplicator_;						// membrane potential equals membrane potential times exponential decay 
	else																						// plus exponentially decaying excitatory / inhibitory input current times stability factor (receptor type 1)
      --S_.r_ref_; // neuron is absolute refractory		
      
    S_.i_old_ = S_.i_syn_ex_;																// inserted: safe current from last time step								
