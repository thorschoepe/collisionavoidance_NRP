...
private:
  ...
  struct Parameters_2
  {
    ...
    Parameters_2(); //!< Sets default parameter values
  ...
  struct State_
  {
    ...
    double i_old_;   
    double multiplicator_; 																				// newly inserted variable for old current value
    ...
    void get( DictionaryDatum&, const Parameters_2& ) const;
     ...
     */
    void set( const DictionaryDatum&, const Parameters_2&, const double );
  };
...
inline nest::port
mynest::iaf_psc_exp_semd::handles_test_event( nest::SpikeEvent&, nest::rport receptor_type )
{
  if ( receptor_type != 0 )
...
inline nest::port
mynest::iaf_psc_exp_semd::handles_test_event( nest::CurrentEvent&, nest::rport receptor_type )
{
  if ( receptor_type == 0 )
    return 0;
...
inline nest::port
mynest::iaf_psc_exp_semd::handles_test_event( nest::DataLoggingRequest& dlr, nest::rport receptor_type )
{
  if ( receptor_type != 0 )
    throw nest::UnknownReceptorType( receptor_type, get_name() );
...
{
  Parameters_2 ptmp = P_;                 // temporary copy in case of errors
